...
trait MLWritable {
   ...
   * Returns an SparkRandomForestRegressionModelMLWriter instance for this ML instance.
    * 返回此ML实例的SparkRandomForestRegressionModelMLWriter实例
   */
  @Since("1.6.0")
  def write: MLWriter
   ...
   * Saves this ML instance to the input path, a shortcut of `write.save(path)`.
    * 将这个ML实例保存到输入路径,write.save（path）的快捷方式
   */
  @Since("1.6.0")
  @throws[IOException]("If the input path already exists but overwrite is not enabled.")
...
trait MLReadable[T] {
   ...
   * Returns an SparkRandomForestRegressionModelMLReader instance for this class.
    * 返回此类的SparkRandomForestRegressionModelMLReader实例
   */
  @Since("1.6.0")
  def read: MLReader[T]
   ...
   * Reads an ML instance from the input path, a shortcut of `read.load(path)`.
    * 从输入路径读取一个ML实例，一个`read.load（path）`的快捷方式
   *
...
private[ml] object DefaultParamsReader {
   ...
   * @param metadata  All metadata, including the other fields 所有元数据,包括其他字段
   ...
   * @throws IllegalArgumentException if expectedClassName is specified and does not match metadata
    *                                  如果expectedClassName被指定,并且不符合元数据
   */
   ...
   * Parse metadata JSON string produced by SparkRandomForestRegressionModelDefaultParamsWriter.getMetadataToSave().
    * 解析由SparkRandomForestRegressionModelDefaultParamsWriter.getMetadataToSave（）生成的元数据JSON字符串
   * This is a helper function for SparkRandomForestRegressionModelloadMetadata().
    * 这是SparkRandomForestRegressionModelloadMetadata（）的一个辅助函数。
   *
   * @param metadataStr  JSON string of metadata JSON字符串的元数据
   * @param expectedClassName  If non empty, this is checked against the loaded metadata.
    *                           如果不为空,则根据加载的元数据进行检查
   * @throws IllegalArgumentException if expectedClassName is specified and does not match metadata
   */
   ...
   * Load a SparkRandomForestRegressionModelParams instance from the given path, and return it.
    * 从给定的路径加载一个SparkRandomForestRegressionModelParams实例,并返回它
