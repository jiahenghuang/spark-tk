...
object LogisticRegressionModel extends TkSaveableObject {
   ...
   * 创建一个逻辑回归模型,并使用给定框架的观察列和标签列进行训练
  ...
  def train(frame: Frame,
    ...
    val labeledTrainRdd = null;//frameRdd.toLabeledPointRDDWithFrequency(labelColumn, observationColumns.toList, frequencyColumn)
    ...
    val trainingSummary =null// buildSummaryTable(frame.rdd.sparkContext, sparkLogRegModel, observationColumns, intercept, mlModel.getHessianMatrix)
