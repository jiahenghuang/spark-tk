...
trait PowerIterationClusteringSummarization extends BaseFrame {
   ...
   * 执行Power Iteration Clustering以创建小于或等于'k'个群集,返回分类为簇的数据以及簇的数量和簇的映射以及各自的大小
   * @param sourceColumn Name of the column containing the source node 包含源节点的列的名称
   * @param destinationColumn Name of the column containing the destination node 包含目标节点的列的名称
   * @param similarityColumn Name of the column containing the similarity 包含相似性的列的名称
   * @param k Number of clusters to cluster the graph into. Default is 2 将图形聚类成的群集数量。 默认值是2
