...
class BinColumnTest extends TestingSparkContextWordSpec with Matchers {
  ...
  "binEqualWidth" should {
    ...
    //如果num_bins大于列的长度，则将每个元素放在单独的bin中
  ...
  "binEqualDepth" should {
    ...
    //在相同的箱子中将相同的数值相同,即使这意味着创建少于所需的箱子
    ...
    //如果num_bins大于列的长度,则将每个值放在单独的bin中
  ...
  "binColumn" should {
    //strictBinning为false时，将截断值以外的值放入最后一个bin的第一个
    ...
    //strictBinning为true时,将截断值以外的值放入bin -1
