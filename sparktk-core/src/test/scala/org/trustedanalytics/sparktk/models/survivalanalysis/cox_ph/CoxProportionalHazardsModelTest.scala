...
class CoxProportionalHazardsModelTest extends TestingSparkContextWordSpec with Matchers {
  ...
  "CoxProportionalHazardsModel train" should {
    //从训练中创建一个CoxProportionalHazardsModel比例风险模型
    ...
    //在训练中为空的协变量列抛出IllegalArgumentException
    ...
    //在训练中的空闲时间抛出IllegalArgumentException
    ...
    //在训练中为空检查员抛出IllegalArgumentException
  ...
  "CoxProportionalHazardsModel save" should {
    //保存CoxProportionalHazardsModel模型
  ...
  "CoxProportionalHazardsModel exportToMar" should {
    //导出CoxProportionalHazardsModel模型并返回模型路径
